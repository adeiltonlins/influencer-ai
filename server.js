...
app.get('/api/health', (_req, res) => res.json({ ok:true, service:'influencer-ai', version:'0.3.1', geminiConfigured:Boolean(process.env.GEMINI_API_KEY), model:process.env.GEMINI_MODEL||'gemini-3.6-flash' }));
app.get('/api/influencers', (_req,res)=>res.json([{id:'maya',name:'Maya',style:'tech + fitness',voice:'pt-BR'},{id:'sofia',name:'Sofia',style:'lifestyle + beauty',voice:'pt-BR'},{id:'ana',name:'Ana',style:'business + UGC',voice:'pt-BR'}]));
...
const port=Number(process.env.PORT||10000);app.listen(port,'0.0.0.0',()=>console.log(`Influencer AI listening on ${port}`));
